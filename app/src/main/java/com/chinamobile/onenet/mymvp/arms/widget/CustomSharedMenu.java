...
import static com.chinamobile.onenet.mymvp.arms.utils.UiUtils.getColor;

/**
 * Created by Jake on 2016/9/12.
 * Perhaps I would be single for the rest of my life
 */
public class CustomSharedMenu extends ViewGroup implements View.OnClickListener,
    ...
    private CustomSharedButton mSwitchBtn;
    ...
    private void init() {
        mSwitchBtn = new CustomSharedButton(getContext());
    ...
    public CustomSharedMenu(Context context, AttributeSet attrs) {
        super(context, attrs);
        mOuterPadding = dip2px(context, DEFAULT_PADDING);
        mInnerPadding = dip2px(context, DEFAULT_PADDING);
        mVerticalPadding = dip2px(context, DEFAULT_PADDING);
        mSwitchBtnSize = dip2px(context, DEFAULT_BTN_SIZE);
        mDotRadius = dip2px(context, 1);
        mDotDistance = dip2px(context, 25);
        mDotColor = getColor(Color.WHITE);
        mShadowColor = getColor(Color.parseColor("#40000000"));
        mBackgroundColor = getColor(Color.parseColor("#b4282d"));
    ...
    public void onClick(View view) {
        if (view instanceof CustomSharedButton) {
            toggle();
        }
    }
